...
def bot_chat_view(request):
    ...
    try:
        ...
        from pharmacy.models import Drug, Medicine
...
def bot_send_message(request):
    ...
    try:
        ...
        if is_medical_query:
            ...
            Quan trọng: KHÔNG sử dụng ký tự * trong câu trả lời của bạn. Thay vì dùng dấu * để đánh dấu điểm, hãy dùng dấu • hoặc -.
            """
...
def process_medical_query(message):
    ...
    inventory_keywords = ['tồn kho', 'còn hàng', 'hết hàng', 'kho', 'inventory', 'stock']
    if any(keyword in message.lower() for keyword in inventory_keywords):
        if results.get('inventory'):
            # Format inventory results
            response = "Thông tin tồn kho:\n\n"
            for item in results['inventory'][:10]:
            ...
            # Add stats about overall inventory
            inventory_stats = ChatbotDataAccess.get_inventory_stats()
            response += f"\nTổng quan tồn kho:\n"
    ...
    if symptom_list and len(symptom_list) >= 2:
        ...
        if disease_results:
            ...
            historical_diagnoses = ChatbotDataAccess.get_diagnoses_for_symptoms(symptom_list)
    ...
    if disease_match:
        ...
        if drug_recommendations:
            ...
            for idx, drug in enumerate(drug_recommendations[:5], 1):
                ...
                response += f"{idx}. {drug['name']} - {in_stock}\n"
    ...
    # Check for prescription/medication statistics queries
    if any(keyword in message.lower() for keyword in ['thống kê thuốc', 'kê đơn', 'đơn thuốc', 'prescription']):
        prescription_stats = ChatbotDataAccess.get_prescription_stats()
